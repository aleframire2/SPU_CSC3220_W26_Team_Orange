...
void run_export_menu(sqlite3* db, int user_id) {
    ...
    if (stories.empty()) {
        std::cout << "  No stories to export.\n";
        return;
    }

    std::cout << "\n  Your stories:\n";
    for (auto& s : stories)
        std::cout << "  [" << s.story_id << "] " << s.title << "\n";
    ...
    if (ok) std::cout << "  Exported to: " << out_path << "\n";
    else    std::cout << "  [!] Failed to write file.\n";
}
